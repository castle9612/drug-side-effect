...
dataset = pd. read_csv('../drug-side-effect-shared-datasets/datasets_target_smiles_approval.csv')
# print(dataset)
...
class AUTO_ENCODER_target(torch.nn.Module):
    def __init__(self,args):
        super(AUTO_ENCODER_target, self).__init__()
...
target_action_data = dataset.drop(['Name','ApprovalDate','SMILES','drug_id','THROMBOCYTOPENIA'], axis = 1)

X_target = target_action_data
dataset_auto_target = target_actionDataset(X_target)
dataloader_auto_target = DataLoader(dataset_auto_target, batch_size=args.batch_size, shuffle = False)

print(X_target)
X_target = torch.tensor(X_target.values, dtype=torch.float32)
model_auto_target = AUTO_ENCODER_target(args)
optimizer_auto_target = torch.optim.Adam(model_auto_target.parameters(), lr=1e-5)


for epoch in range(10):
    print(f"Epoch {epoch}\n ---------")
    train_model_auto(dataloader_auto_target, model_auto_target, loss_mse, optimizer_auto_target)
    print("")

X_eco1, X_dco1 = model_auto_target(X_target)
...
X_eco1 = X_eco1.detach().numpy()
X_eco1 = pd.DataFrame(X_eco1)
X_eco1.columns = [f"encoded1_{i}" for i in range(X_eco1.shape[1])]
...
for adr_name in adr_name_list:
    ...
    # y = y.tail(len(y)-128)
    y = y.to_numpy()
    X_final_train, X_final_test, y_final_train, y_final_test = train_test_split(X, y, stratify=y, test_size=0.36)
...
file_name = 'test_xgboost_0715_result_4fold_smote_smiles_target_128' + adr_name + '.csv'
